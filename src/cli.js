...
program.command('dao-vote <id> <amount>')
    .alias('proposal-vote')
    .description('vote for a dao proposal')
    .action(function(id, amount) {
        verifyAndSendTx('daoVote', id, amount)
    }).on('--help', function(){
        writeLine('')
        writeLine('Arguments:')
...
program.command('fundrequest-create <requested> <receiver> <json> <deadline>')
    .description('create a new funding request')
    .action(function(requested, receiver, json, deadline) {
        verifyAndSendTx('fundRequestCreate', requested, receiver, json, deadline)
    }).on('--help', function(){
        writeLine('')
        writeLine('Arguments:')
        writeLine('  <requested>: requested amount for this fund request')
        writeLine('  <receiver>: beneficiary of the fund request')
        writeLine('  <json>: json metadata of the fund request')
        writeLine('  <deadline>: unix timestamp (in ms) of deadline of the proposal before it expires (max 1 year after funding period ends)')
        writeLine('')
        writeLine('Examples:')
